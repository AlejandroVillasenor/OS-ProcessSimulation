...
int main(){
	system("title Programa 4 - Algoritmo de planificacion FCFS - Continuacion"); //Configuración consola
	...
	int initialProcess = 0,indexProcess = 0, finishedProcess = 0, indexExP = 0; //Identifica el proceso a continuación
    ...
    do{
        system("cls");
		cout<<"|| Algoritmo de planificacion FCFS - Continuacion ||"<<endl;
        cout<<"Numero de procesos a registrar: ";
        getline(cin, numPro);
        numProcessVal = validatePI(&numPro, 2);
        if(numProcessVal){
            initialProcess = stoi(numPro);
            (initialProcess <= 0 || initialProcess >= 30) ? numProcessVal = false : numProcessVal = true ; 
            if(numProcessVal){
                cout<<errmsg;
            }
        }else{
